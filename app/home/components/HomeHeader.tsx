...
const callsToAction = [
    ...
    { name: 'Cerrar Sesión', icon: HiOutlineLogin, action: handleLogout },
]
...
export const HomeHeader: React.FC<UserProfileProps> = ({ maxWords = 2 }) => {

    const { data: session } = useSession();

    const getShortenedName = (name: string | undefined | null): string => {
        if (name) {
                    ...
                    <Popover className="relative">
                        ...
                        >
                            <Popover.Panel className="absolute -left-32 top-full z-10 mt-3 w-screen max-w-md overflow-hidden rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5">
                                ...
                                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50 cursor-pointer">
                                        ...
                                        <>
                                            <Disclosure.Button className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-semibold leading-7 hover:bg-gray-50">
                                                <div className='flex'>
                                                    <Avatar img={session?.user?.image ?? ''} />
                                                    {session?.user ? (
                                                        <span className="flex items-center ml-3">{getShortenedName(session.user.name)}</span>
                                                    ) : null}
                                                </div>
                                                <HiChevronDown
